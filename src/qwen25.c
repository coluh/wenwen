...
float* malloc_fp32(int size, uint16_t* bf16data) {
...
void* Qwen25_05B(ModelConfig* config, const char* model_safetensors) {
	...
	m->embedding.table = malloc_fp32(V * D, get_tensor(sf, "model.embed_tokens.weight"));
	...
	for (int l = 0; l < config->num_layers; l++) {
		...
		layer->norm.weight = malloc_fp32(D, get_tensor(sf, buffer));
		layer->norm.eps = config->rms_norm_eps;

		sprintf(buffer, "model.layers.%d.self_attn.q_proj.weight", l);
		layer->attention.q.weight = malloc_fp32(D * D, get_tensor(sf, buffer));
		sprintf(buffer, "model.layers.%d.self_attn.k_proj.weight", l);
		layer->attention.k.weight = malloc_fp32(D * Hkv * Dh, get_tensor(sf, buffer));
		sprintf(buffer, "model.layers.%d.self_attn.v_proj.weight", l);
		layer->attention.v.weight = malloc_fp32(D * Hkv * Dh, get_tensor(sf, buffer));
		sprintf(buffer, "model.layers.%d.self_attn.o_proj.weight", l);
		layer->attention.o.weight = malloc_fp32(D * D, get_tensor(sf, buffer));

		sprintf(buffer, "model.layers.%d.post_attention_layernorm.weight", l);
		layer->post_norm.weight = malloc_fp32(D, get_tensor(sf, buffer));
		layer->post_norm.eps = config->rms_norm_eps;

		sprintf(buffer, "model.layers.%d.mlp.gate_proj.weight", l);
		layer->mlp.gate.weight = malloc_fp32(D * Df, get_tensor(sf, buffer));
		sprintf(buffer, "model.layers.%d.mlp.up_proj.weight", l);
		layer->mlp.up.weight = malloc_fp32(D * Df, get_tensor(sf, buffer));
		sprintf(buffer, "model.layers.%d.mlp.down_proj.weight", l);
		layer->mlp.down.weight = malloc_fp32(D * Df, get_tensor(sf, buffer));
	}

	m->norm.weight = malloc_fp32(D, get_tensor(sf, "model.norm.weight"));
	...
	for (int pos = 0; pos < seq_len; pos++) {
		for (int h = 0; h < num_heads; h++) {
			for (int m = 0; m < half_dim; m++) {
				...
				int idx = (pos * num_heads + h) * head_dim + 2 * m;
				float x0 = Q[idx];
				float x1 = Q[idx + 1];
				Q[idx] = x0 * cosf(angle) - x1 * sinf(angle);
				Q[idx + 1] = x0 * sinf(angle) + x1 * cosf(angle);
			}
		}
	}
}
...
int Qwen25_05B_inference(Qwen25_05B_Model* model, const int* tokens, int seq_len) {
	...
	for (int l = 0; l < 24; l++) {
		...
		// 2.4 Residual
		// X_mid = X + OW_O
		cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, N, D, D, 1.0f, O, D, W_O, D, 1.0f, R, D);
